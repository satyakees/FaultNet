...
def main(args):
    ...
    print(" ---teacher model is at %s ---  "%(args.model_teacher))
    ...
    model_teacher = load_models.getModel(model_arch=model_arch,output_channels=output_channels,parallel_flag=parallel_flag)
    checkpoint = torch.load(args.model_teacher)
    model_teacher.load_state_dict(checkpoint['model_state'])
    model_teacher.eval()
    print(" ------ loaded pretrained model as teacher -----------")
    ...
    for epoch in range(nepoch):
        ...
        for i, (images, labels) in enumerate(training_data_loader):
            ...
            ## teacher pass:
            with torch.no_grad():
                pred_teacher = model_teacher(images.float().cuda())
        ...
        total_mseloss = total_mseloss/len(training_data_loader)
...
if __name__=="__main__":
    ...
    parser.add_argument('-tm', '--model-teacher', type=str, metavar='DIR', help='Path of teacher model', required=True)
